...
qR = obj.qR;
numberOfInternalNodes = size(mixedFEObject.shapeFunctionObject.N_k_I, 2);

% Electric Displacement
% FIXME different shapefunctions for mixed fields
N_D = mixedFEObject.shapeFunctionObject.N_k_I;
N_C = mixedFEObject.shapeFunctionObject.N_k_I;
N_G = mixedFEObject.shapeFunctionObject.N_k_I;
N_c =  mixedFEObject.shapeFunctionObject.N_k_I;
...
% Output Variable
out(numberOfElements) = struct('QN1',[],'ana',[],'delta',[],'phi_analytical',[],'delta_phi',[],'theta_analytical',[],'delta_theta',[],'DN1',[],'delta_D',[],'D_analytical',[]);
...
for e = 1:numberOfElements
    ...
    ana_theta = analyticalVariablesCell.q(edofLocal,dimension+2)';
    ...
    edR = obj.qR(edofLocal, 1:dimension)';
    % Run through all Gauss points
    for k = 1:numberOfGausspoints
        dN_xi_I = reshape(dN_xi_k_I(:,k,:),[size(dN_xi_k_I,1),size(dN_xi_k_I,3)]);
        [~,detJ] = computeJacobian(edR,dN_xi_I,10*eps,false);
        
        Phi = qN1(:,1:dimension)'*N_k_I(k,:)';
        Phi_ana = ana_q*N_k_I(k,:)';
        delta_q = Phi-Phi_ana;        
        ...
        phi = qN1(:,dimension+1)'*N_k_I(k,:)';
        phi_ana = ana_Phi*N_k_I(k,:)';
        ...
        theta = qN1(:,dimension+2)'*N_k_I(k,:)';
        theta_ana = ana_theta*N_k_I(k,:)';
        delta_theta = theta-theta_ana;
        % Summation over all Gauss Points
        save_thetaN1 = save_thetaN1 + theta'*theta*detJ*gaussWeight(k);
        save_theta_analytical = save_theta_analytical + theta_ana'*theta_ana*detJ*gaussWeight(k);
        save_delta_theta = save_delta_theta + delta_theta'*delta_theta*detJ*gaussWeight(k);
    ...
    out(e).thetaN1 = save_thetaN1;
    out(e).theta_analytical = save_theta_analytical;
    out(e).delta_theta = save_delta_theta;
