...
[plateObject.meshObject.nodes,plateObject.meshObject.edof, edofBoundary] = meshRectangle(plateLength, plateLength, 1, 1, orderShapeFunctions);
plateObject.meshObject.nodes = plateObject.meshObject.nodes + plateLength/2;
% plateObject.meshObject.nodes(1,:) = [1, 1];
% plateObject.meshObject.nodes(2,:) = [8, 3];
% plateObject.meshObject.nodes(3,:) = [2, 14];
% plateObject.meshObject.nodes(4,:) = [9, 16];
% plateObject.meshObject.nodes(1,:) = [-1, -1];
% plateObject.meshObject.nodes(2,:) = [1, -1];
% plateObject.meshObject.nodes(3,:) = [-1, 1];
% plateObject.meshObject.nodes(4,:) = [1, 1];
delta = 0;
plateObject.meshObject.nodes(1,:) = [-1 - delta, -1- delta];
plateObject.meshObject.nodes(2,:) = [1- delta, -1+ delta];
plateObject.meshObject.nodes(3,:) = [-1+ delta, 1- delta];
plateObject.meshObject.nodes(4,:) = [1+ delta, 1+ delta];
plateObject.meshObject.nodes(4,:) = plateObject.meshObject.nodes(4,:)*3;
...
% neumannObject.forceVector = -[100; 0; 0];                         % Komponenten des Kraftvektors anpassen
...
% neumannObject.shapeFunctionObject.numberOfGausspoints = 4;  % Anzahl der Gaußpunkten auf dem Neumannrand anpassen
% neumannObject.meshObject.edof = plateObject.meshObject.edof;

neumannObject = neumannClass(dofObject);
neumannObject.loadGeometry = 'area';
neumannObject.masterObject = plateObject;
neumannObject.loadVector = -[10; 0; 0]; % Komponenten des Kraftvektors anpassen
neumannObject.meshObject.edof = plateObject.meshObject.edof;
