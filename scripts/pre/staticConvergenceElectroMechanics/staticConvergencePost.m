...
if strcmpi(solidElectroObject.elementDisplacementType,'mixedSC')
    errorL2 = zeros(size(All_Elem,2),9);
else
    errorL2 = zeros(size(All_Elem,2),3);
end
%% Loop over all discretisations
for j = 1:size(All_Elem,2)
    out = error_calculation_routine(journalSolidElectroObject{j},analyticalVariablesCell{j});
    ...
    errorL2(j,3) = sqrt(sum(vertcat(out.delta_D)))/sqrt(sum(vertcat(out.D_analytical)));
    if strcmpi(solidElectroObject.elementDisplacementType,'mixedSC')
        errorL2(j,4) = sqrt(sum(vertcat(out.delta_C)))/sqrt(sum(vertcat(out.C_analytical)));
        errorL2(j,5) = sqrt(sum(vertcat(out.delta_G)))/sqrt(sum(vertcat(out.G_analytical)));
        errorL2(j,6) = sqrt(sum(vertcat(out.delta_c)))/sqrt(sum(vertcat(out.c_analytical)));
        errorL2(j,7) = sqrt(sum(vertcat(out.delta_lambdaC)))/sqrt(sum(vertcat(out.lambdaC_analytical)));
        errorL2(j,8) = sqrt(sum(vertcat(out.delta_lambdaG)))/sqrt(sum(vertcat(out.lambdaG_analytical)));
        errorL2(j,9) = sqrt(sum(vertcat(out.delta_lambdac)))/sqrt(sum(vertcat(out.lambdac_analytical)));
    end
end
...
legend('x','\Phi','D','C','G','c','\Lambda_C','\Lambda_G','\Lambda_c')%,'location','west')
set(gca,'fontsize',16)
a1 = 1;
ae = size(errorL2,1);
