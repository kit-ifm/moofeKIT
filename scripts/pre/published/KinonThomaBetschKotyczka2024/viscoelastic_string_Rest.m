...
%
% problem: visco-hyperelastic string, fixed on top,
%          (quasi-)static case
% spatial discretization: linear Lagrange shapefunction for disp
%                         discont. constant ansatz for strain, 
%                         mixed formulation
% time discretization: endpoint (required for steady state)
%
% user: Philipp Kinon
...
if new_calculation
    ...
    % Specify initial guess for strains different from 1 (required for static case)
    dofObject.listContinuumObjects{1}.mixedFEObject.qN = 1.5*ones(numberOfElementsOfCrime,2);
...
%Export state for dynamic simulation
nodesEquilibrium = reshape(dofObject.qN1(1:2*(numberOfElementsOfCrime+1)),[2,numberOfElementsOfCrime+1])';
mixedEquilibrium = [dofObject.qN1(2*(numberOfElementsOfCrime+1)+1:2:end),dofObject.qN1(2*(numberOfElementsOfCrime+1)+2:2:end)];
