...
nGP = 2;
%reduced_nGP = 1;
allTimestepsizes = [0.1, 0.05, 0.02, 0.01, 0.005];
totalTime = 1;
...
x = zeros(numel(allTimestepsizes),1);
y = x;

for index = 1:numel(allTimestepsizes)
    timestepsize = allTimestepsizes(index);
    ...
    x(index) = dofObject.postDataObject.stateJournal(end).position(numberOfDOFFirstBeam+3*3+1);
    y(index) = dofObject.postDataObject.stateJournal(end).position(numberOfDOFFirstBeam+3*3+2);
end
x_ref = 1.396144818042612;
y_ref = -0.763824524916379;

%assert(norm(x_ref - x(end)) < 1e-14,'Recompute reference results!')
%assert(norm(y_ref - y(end)) < 1e-14,'Recompute reference results!')

error = zeros(numel(allTimestepsizes)-1,1);
for index2 = 1:numel(allTimestepsizes)-1
    error(index2) = norm([x(index2);y(index2)]-[x_ref;y_ref])/norm([x_ref;y_ref]);
end
figure();
loglog(allTimestepsizes(1:end-1),error);
%matlab2tikz('height', '\figH', 'width', '\figW', 'filename', 'time_convergence.tikz', 'showInfo', false, 'floatformat', '%.4g')
