function [x,phi,theta,D,C,G,c,lambdaC,lambdaG,lambdac,B,rho0,R] = staticConvergenceAnalyticalComputations(a,b,c1,d1,c2,d2,e0,er,kappa,beta,k0,g1,g2,g3,phi0,thetaR,thetaRb,choiceFunction,dimension)
...
fTheta = theta / thetaR;
fTheta = fTheta(X1,X2,X3);

% electrical quantities
E = -GradientOfScalar(phi);
E = E(X1,X2,X3);
D = er*e0*1/fTheta*c^(1/2)*(CInv*E);

% thermal quantities
Gamma = -GradientOfScalar(theta);
Gamma = Gamma(X1,X2,X3);
Q = k0/c*G*Gamma;

% constitutive equations
%     Psi = a*(trace(Cana)-3) + b*(trace(Gana)-3) + c1/2*(sqrt(cana)-1)^2 - d1*log(sqrt(cana)) - n_dim*beta*c2*(thetaN1e - theta0)*(cxN1 - 1) + 1/(2*er*e0*cana^(1/2))*Dana'*Cana*Dana +  kappa*(theta - theta0 - theta*log(thetaN1e/theta0));
...
DW_C = fTheta*(a*eye(3) + 1/(2*er*e0*sqrt(c))*(D*D'));
DW_G = fTheta*b*eye(3);
DW_c = -dimension*beta*c2*(theta - thetaR) + fTheta*(c1/2*(1-1/sqrt(c)) - d1/(2*c) - 1/(4*er*e0*c^(3/2))*D'*(C*D));
