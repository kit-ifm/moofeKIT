...
from extractEdofPython import extractEdofPython
...
edofRandRechts = edof[-elemente[1]:, 2:6] - 1
edRandRechts = extractEdofPython(edofRandRechts, q)

edof = edof - 1
# Geometriedatentabellen
# bzgl. Gebiet (2D Elemente)
ed = extractEdofPython(edof, q)
...
for i in np.arange(0, nel):
    ...
    dataFE['indexKi'][i] = (np.kron(np.ones(edof.shape[1]),edof[i,:])).astype('i')
    dataFE['indexKj'][i] = (np.kron(edof[i,:],np.ones(edof.shape[1]))).astype('i')
    dataFE['Fe'][i] = Fe
    dataFE['indexFi'][i] = (edof[i,:]).astype('i')
...
for i in np.arange(0, elemente[0]):
    ...
    F = assem(F, Fe, edofRandRechts[i, :]+1)
...
ed_neu = extractEdofPython(edof, q)
draw(ed_neu)

## Nachlaufrechnung fuer Spannungsberechnung
d_neu = extractEdofPython(edof, d)
