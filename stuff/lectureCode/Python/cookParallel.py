...
from elem_loop import elem_loop
...
elemente[0] = 100    # in x-Richtung
elemente[1] = 100    # in y-Richtung
...
dataParallel = Parallel(n_jobs=24)(delayed(elem_loop)(i, ed, edof, ir, C) for i in range(nel))

for i in np.arange(0, np.size(dataParallel)):
    dataFE['KeV'][i] = dataParallel[i]['KeV']
    dataFE['indexKi'][i] = dataParallel[i]['indexKi']
    dataFE['indexKj'][i] = dataParallel[i]['indexKj']
    dataFE['Fe'][i] = dataParallel[i]['Fe']
    dataFE['indexFi'][i] = dataParallel[i]['indexFi']

# sparse assembling
K = csr_matrix((np.concatenate(dataFE['KeV']),
                (np.concatenate(dataFE['indexKi']),
                  np.concatenate(dataFE['indexKj']))))
F = csr_matrix((np.concatenate(dataFE['Fe']),
                (np.concatenate(dataFE['indexFi']),
                  np.zeros(np.size(np.concatenate(dataFE['indexFi'])),
                          dtype="i"))))
...
#     x[0:4, i], y[0:4, i], se, He = nachlauf(ed[i, :], ed_neu[i, :],
#                                             d_neu[i, :], ir, C)
