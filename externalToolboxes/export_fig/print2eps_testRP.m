    ...
    if using_hg2 && ~isempty(hAxes)  % issue #211 presumably happens only in HG2, not HG1
        try
            ...
            if any(~strcmpi('ChildOrder',oldSortMethods))  % i.e., any oldSortMethods=='depth'
                %MODIFICATION ROBIN PFEFFERKORN: 
                %Seems to be unnecessary and just costs time
...
function [StoredColors, fstrm, foundFlags] = eps_maintainAlpha(fig, fstrm, StoredColors)
    if nargin == 1  % in: convert transparency in Matlab figure into unique RGB colors
        ...
        for objIdx = 1:length(hObjs)
            ...
            for propIdx = 1 : numel(propNames)
                try
                    ...
                    %MODIFICATION Robin Pfefferkorn handle transparency of
                    %patches with mapped color
                    elseif strcmp(hObj.(propName).ColorType, 'colormapped')
                        oldColor = hObj.(propName).Texture.CData;
                        if size(oldColor,1)>3 && any(oldColor(4,:)~=255)  % issue #281: only fix patch/textbox color if it's not opaque
                            nColors = length(StoredColors):(length(StoredColors)+size(oldColor,2)-1);
                            newColor = uint8([
                                101*ones(1,size(oldColor,2)); 
                                102+floor(nColors/255); 
                                mod(nColors,255); 
                                255*ones(1,size(oldColor,2))]);
                            %additonal empty cells to include all new
                            %colors
                            StoredColors{end+256} = {hObj, [propName,'Mapped'], oldColor, newColor}; %#ok<AGROW>
                            hObj.(propName).Texture.CData = newColor;
                        end
                    end
                catch
        ...
        %delete empty cells
        StoredColors = StoredColors(~cellfun('isempty',StoredColors));
        ...
        for objIdx = 1 : nColors
            ...
            newColor  = colorsData{4};
            try
                %MODIFICATION Robin Pfefferkorn: allows to have multiple
                %colors in one orig/new color block
                for colIndx=1:size(origColor,2)
                    %Restore the EPS files patch color
                    colorID   = num2str(round(double(newColor(1:3,colIndx)') /255,3),'%.3g %.3g %.3g'); %ID for searching
                    origRGB   = num2str(round(double(origColor(1:3,colIndx)')/255,3),'%.3g %.3g %.3g'); %Replace with original color
                    origAlpha = num2str(round(double(origColor(end,colIndx)) /255,3),'%.3g'); %Convert alpha value for EPS
                    ...
                    if strcmpi(propName,'Face') || strcmpi(propName,'FaceMapped')
                    ...
                    if strcmpi(propName,'FaceMapped')
                        hObj.Face.Texture.CData = origColor;
                    else
