...
%    end
    ...
    try
        ...
        if isvector(options)
            ...
            %modification to avoid double copying of pdf
            [fpath,fname] = fileparts(options.name);
            if isTempDirOk && (contains(fullfile(fpath,[fname '.pdf']),'%') || options.eps)
            ...
            try
                ...
                if options.transparent %&& ~isequal(get(fig, 'Color'), 'none')
                    %MODIFICATION ROBIN PFEFFERKORN
                    %the following line is not necessary anymore
                    %(transparency automatically adapted with
                    %fig.Color='none' for painter renderer)
                    if options.renderer~=3
...
function [fig, options] = parse_args(nout, fig, argNames, varargin)
    ...
    if isequal(get(ancestor(fig(1), 'figure'), 'Color'), 'none')
        %MODIFICATION ROBIN PFEFFERKORN: 
        %Apperently seting the transparency is not necessary and very time
        %consuming --> thus switched of
%         options.transparent = true;
    end
