...
Als Zweites wird der Dirichlet-Rand definiert indem in Zeile 25 die \code{dirichletClass} aufgerufen wird und das \code{dirichletObject} erstellt. Das dritte Objekt ist das \code{neumannObject}, welches aus der \code{neumannClass} in Zeile 32 erstellt wird, und beinhaltet die Formfunktionen, Gaußpunkte und Freiheitsgrade f\"ur den Neumannrand sowie die angreifende Kraft und deren zeitlicher Verlauf.

Danach wird die Funktion \code{runNewton.m} in Zeile 42 aufgerufen. Diese wird im n\"achsten Abschnitt genauer erkl\"art. Zum Schluss wird im Startskript die Verschiebung des rechten oberen Knotens ausgegeben.
