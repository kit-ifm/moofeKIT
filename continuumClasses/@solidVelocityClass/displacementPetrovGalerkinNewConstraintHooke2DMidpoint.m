function [rData, kData, elementEnergy, array] = displacementPetrovGalerkinNewConstraintHooke2DMidpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
% DISPLACEMENTPETROVGALERKINNEWCONSTRAINTHOOKE2DMIDPOINT Element routine of class solidVelocityClass.
%
% FORMULATION
...
%
% CALL
% displacementPetrovGalerkinNewConstraintHooke2DMidpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
        % RESIDUAL
        RX = RX + (NMat.' * rho * NMat * 1/deltaT *(vN1(:) - vN(:)) + B' * C * L * uN05(:)) * detJ * gaussWeight(k);
        RV = RV + (L.' * C * (L * 1/deltaT *(uN1(:) - uN(:)) - B * vN05(:))) * detJ * gaussWeight(k);

        % TANGENT
        KXX = KXX + 1/2 * B' * C * L * detJ * gaussWeight(k);
        KXV = KXV + NMat.' * rho * NMat * 1/deltaT * detJ * gaussWeight(k);
        KVX = KVX + L.' * C * L * 1/deltaT * detJ * gaussWeight(k);
