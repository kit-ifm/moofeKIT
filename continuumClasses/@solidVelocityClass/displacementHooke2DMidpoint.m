...
% DISPLACEMENTHOOKE2DMIDPOINT Element routine of class solidVelocityClass.
%
% FORMULATION
...
%
% CALL
% displacementHooke2DMidpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
        ...
        RV = RV + (NMat.' * rho * (1/deltaT * NMat *(uN1(:) - uN(:)) - NMat*vN05(:))) * detJ * gaussWeight(k);

        % TANGENT
        KXX = KXX + 1/2 * B' * C * B * detJ * gaussWeight(k);
        KXV = KXV + NMat.' * rho * NMat * 1/deltaT * detJ * gaussWeight(k);
        KVX = KVX + NMat.' * rho * NMat * 1/deltaT * detJ * gaussWeight(k);
        KVV = KVV - 1/2 * NMat.' * rho * NMat * detJ * gaussWeight(k);
        ...
        elementEnergy.dispError = elementEnergy.dispErrorPos + ((NMat * uN1(:))' * (NMat * uN1(:))) * detJ * gaussWeight(k);
    else
        % STRESS COMPUTATION
        sigmaN1_v = C * B * uN1(:);
