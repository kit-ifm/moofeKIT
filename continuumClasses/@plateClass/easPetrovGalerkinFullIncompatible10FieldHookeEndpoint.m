function [rData, kData, elementEnergy, array] = easPetrovGalerkinFullIncompatible10FieldHookeEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, ~)
...
for k = 1:numberOfGausspoints
    ...
    Er(4, 3) = xi*eta-1/3*C2*eta;
    Er(5, 4) = xi*eta-1/3*C1*xi;
    
    Er(4, 5) = 3/2*xi^2-1/2;
    Er(5, 6) = 3/2*eta^2-1/2;
    ...
    Er(5, 3) = 3/2*xi^2-1/2;
    Er(4, 4) = 3/2*eta^2-1/2;
    
    Er(4, 7) = 3/2*xi^2-1/2;
    Er(4, 8) = 3/2*eta^2-1/2;
    Er(5, 9) = 3/2*xi^2-1/2;
    Er(5, 10) = 3/2*eta^2-1/2;

    % shape functions for the enhanced part of the strain field
    Hr = zeros(5, 10);
    Hr(4:5, 1:6) = dMTilder(indx, 1:6);
    
    Hr(1, 7:8) = dMTilder(indx(1), 1:2);
    Hr(3, 7:8) = dMTilder(indx(2), 1:2);
    Hr(4, 7:8) = MTilde(k, 1:2);
    
    Hr(2, 9:10) = dMTilder(indx(2), 1:2);
    Hr(3, 9:10) = dMTilder(indx(1), 1:2);
    Hr(5, 9:10) = MTilde(k, 1);
