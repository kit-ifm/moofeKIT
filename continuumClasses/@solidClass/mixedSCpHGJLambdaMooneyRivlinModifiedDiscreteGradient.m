function [rData, kData, elementEnergy, array] = mixedSCpHGJLambdaMooneyRivlinModifiedDiscreteGradient(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    %     DW_C = alpha*trace(CxN05)*(eye(3)-(trace(CxN1)-trace(CxN))/(deltaC(:)'*deltaC(:))*deltaC) + 1/2*(alpha*(trace(CxN1))^2-alpha*(trace(CxN))^2)/(deltaC(:)'*deltaC(:))*deltaC;
    % else
    %     flagDiscreteGradient(k, 1) = 0;
    %     DW_C = alpha*trace(CxN05)*I;
    % end
    deltaJ = JN1 - JN;
    ...
    % else
    %     discreteGradientCondition_J = flagDiscreteGradient(k, 2);
    % end
    ...
    SN05 = 2 * (DW_C + wedge(lambdaGN1, CxN05) + 1/2*lambdaJN1*JxN05^(-1)*GxN05);
    FvN05 = edvN05 * dN_X_I';
    ...
    if ~computePostData
        ...
        % Strain energy
        W = alpha/2 * ((trace(CxN1))^2-9) + beta/2 * ((trace(GN1))^2 - 9) - gamma*log(JN1) + epsilon1*(JN1^(2*epsilon2)+JN1^(-2*epsilon2)-2);
