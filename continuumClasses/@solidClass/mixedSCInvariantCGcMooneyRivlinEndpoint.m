function [rData, kData, elementEnergy, array] = mixedSCInvariantCGcMooneyRivlinEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
extractedIIIN1 = edAlphaN1(2*numberOfInternalNodes+1:3*numberOfInternalNodes);
...
extractedLambdaIIIN1 = edAlphaN1(5*numberOfInternalNodes+1:6*numberOfInternalNodes);
...
for k = 1:numberOfGausspoints
    ...
    % 2nd PK stress
    SN1 = 2 * (lambdaIN1*Imat + lambdaIIN1*wedge(Imat,CxN1) + lambdaIIIN1*GxN1);

    if ~computePostData
        % ENERGY
        elementEnergy.strainEnergy = elementEnergy.strainEnergy + (a * (IN1 - 3) + b * (IIN1 - 3) - d * log(sqrt(IIIN1)) + c / 2 * (sqrt(IIIN1) - 1)^2) * detJ * gaussWeight(k);

        % DERIVATIVES
        DW_I = a;
        DW_II = b;
        DW_III = -d / (2 * IIIN1) + c / 2 * (1 - 1 / sqrt(IIIN1));
        ...
        RIII = RIII + M_k_I(k, :)' * (DW_III - lambdaIIIN1) * detJ * gaussWeight(k);
        ...
        RLambdaIII = RLambdaIII + M_k_I(k, :)' * (cxN1 - IIIN1) * detJ * gaussWeight(k);
        ...
        if 0
            ...
            % KCG
            KCG = KCG + 1 / 3 * kron(M_k_I(k, :)' * M_k_I(k, :), lambdaIIIN1*I6) * detJ * gaussWeight(k);
            ...
            % KGC
            KGC = KGC + 1 / 3 * kron(M_k_I(k, :)' * M_k_I(k, :), lambdaIIIN1*I6) * detJ * gaussWeight(k);
