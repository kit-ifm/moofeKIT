function [rData, kData, elementEnergy, array] = mixedSCNonCascadeGcANNEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
%     DW_III = -d / (2 * cN1) + c / 2 * (1 - 1 / sqrt(cN1));
    [~, ~, dW_III, dW_Ix, dW_II, ~, ~, ~, ~] = ANNObject.computeDiffEnergyCGcANN(CxN1,GN1,cN1);
    ...
    if ~computePostData
        % ENERGY
        WN1 = ANNObject.computeEnergyANN(CxN1,GN1,cN1);
