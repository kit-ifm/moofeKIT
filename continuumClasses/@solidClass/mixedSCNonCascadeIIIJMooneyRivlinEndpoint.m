function [rData, kData, elementEnergy, array] = mixedSCNonCascadeIIIJMooneyRivlinEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
        % ENERGY
        elementEnergy.strainEnergy = elementEnergy.strainEnergy + (a * (IN1 - 3) + b * (IIN1 - 3)  - d * log(JN1) + c / 2 * (JN1 - 1)^2) * detJ * gaussWeight(k);
        ...
        RLambdaJ = RLambdaJ + M_k_I(k, :)' * (JxN1 - JN1) * detJ * gaussWeight(k);
