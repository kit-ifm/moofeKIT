function [rData, kData, elementEnergy, array] = mixedSCNonCascadeGcMooneyRivlinEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
extractedcN1 = edAlphaN1(6*numberOfInternalNodes+1:7*numberOfInternalNodes);
extractedLambdaGN1v = edAlphaN1(7*numberOfInternalNodes+1:13*numberOfInternalNodes);
extractedLambdacN1 = edAlphaN1(13*numberOfInternalNodes+1:14*numberOfInternalNodes);
...
%     [detJ, detJStruct, dN_X_I, ~] = computeAllJacobian(edR,edN,edN1,dN_xi_k_I,k,setupObject);
    ...
    DW_Ix = a;
    DW_II = b;
    DW_III = -d / (2 * cN1) + c / 2 * (1 - 1 / sqrt(cN1));

    % 2nd PK stress tensor
    SN1 = 2*(DW_Ix*I + wedge(lambdaGN1,CxN1) + lambdacN1*GxN1);

    if ~computePostData
        % ENERGY
        elementEnergy.strainEnergy = elementEnergy.strainEnergy + (a * (trace(CxN1) - 3) + b * (trace(GN1) - 3) - d * log(sqrt(cN1)) + c / 2 * (sqrt(cN1) - 1)^2) * detJ * gaussWeight(k);
        ...
        Rc = Rc + M_k_I(k, :)' * (DW_III - lambdacN1) * detJ * gaussWeight(k);
        RLambdaGv = RLambdaGv + kron(M_k_I(k, :)', matrixToVoigt(GxN1-GN1, 'strain')) * detJ * gaussWeight(k);
        RLambdac = RLambdac + M_k_I(k, :)' * (cxN1 - cN1) * detJ * gaussWeight(k);
