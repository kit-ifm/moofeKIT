function [rData, kData, elementEnergy, array] = displacementMooneyRivlinWedgeMidpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
        ...
        % stress
        SN05 = secondPiolaKirchhoff(CxN05, GxN05, detCN05, lambda, a, b); % 2nd PK stress tensor
        SN05V = matrixToVoigt(SN05, 'stress'); % 2nd PK stress tensor in Voigt notation

        % residual
        RG = RG + BN05.' * SN05V * detJ * gaussWeight(k);
        % internal energy
        psiComN1 = internalEnergy(CxN1, GxN1, detN1, lambda, a, b);
        psiComN = internalEnergy(CxN, GxN, detN, lambda, a, b);
        ...
        if analytic
            % analytic
            materialTangent = matTang(CxN05, GxN05, detCN05, lambda, a, b);
            KGG_GP = kron(dN_X_I.'*0.5*SN05*dN_X_I, eye(dimension)) + 2 * BN05.' * materialTangent * BN05;
        else
            % numeric
            for ii = 1:dimR
                edN1Akt = edN1;
...
function [S] = secondPiolaKirchhoff(C, G, detC, lambda, a, b)
DPsiDC = a * eye(3);
DPsiDG = b * eye(3);
...
function [T_mat] = matTang(C, G, detC, lambda, a, b)
