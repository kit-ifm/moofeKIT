function [rData, kData, elementEnergy, array] = mixedSCNonCascadeCGcMooneyRivlinEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    SN1 = 2*(lambdaCN1 + wedge(lambdaGN1,CxN1) + lambdacN1*GxN1);
    ...
    if ~computePostData
        ...
        % RD = RD + BN1.' *  matrixToVoigt(SN1,'strain') * detJ * gaussWeight(k);
        RCv = RCv + kron(M_k_I(k, :)', matrixToVoigt(DW_C-lambdaCN1, 'strain')) * detJ * gaussWeight(k);
        RGv = RGv + kron(M_k_I(k, :)', matrixToVoigt(DW_G-lambdaGN1, 'strain')) * detJ * gaussWeight(k);
