function [rData, kData, elementEnergy, array] = displacementSCIIIJANNDiscreteGradient(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
%     [~,       ~,        ~,       ~,       ~,           dJ_cN1,  dJstar_cN1] = ANNObject.computeDiffEnergyIIIcANN(CN1,GN1,cN1);
    ...
    d2W_IN05 = 0.5*ANNObject.compute2ndDiffEnergyANNCGJ(CN05,GN05,JN05);    
    % discrete gradient
    deltaI = IN1 - IN;
    ...
    if ~flagNumericalTangent
        ...
        if (setupObject.newton.step(setupObject.timeStep) > 1) && (~discreteGradientCondition_I || ~discreteGradientCondition_II || ~discreteGradientCondition_J)
            disp(1)
        end
    else
    ...
    D2W = zeros(4);
    if ~discreteGradientCondition_I || ~discreteGradientCondition_II || ~discreteGradientCondition_J
%         [dW_IN05, dW_IIN05, dW_cN05, dW_JN05, dW_JstarN05, dJ_cN05, dJstar_cN05] = ANNObject.computeDiffEnergyIIIcANN(CN05,GN05,cN05);
        [dW_IN05, dW_IIN05, dW_JN05] = ANNObject.computeDiffEnergyANNJ(CN05,GN05,JN05);
    end
    if discreteGradientCondition_I
        flagDiscreteGradient(k, 1) = 1;
        tempDW_IN1 = ANNObject.computeEnergyANNCGJ(CN1,GN,JN)-ANNObject.computeEnergyANNCGJ(CN,GN,JN);
        tempDW_IN = ANNObject.computeEnergyANNCGJ(CN1,GN1,JN1)-ANNObject.computeEnergyANNCGJ(CN,GN1,JN1);
...
%         D2W(1,1) = D2W(1,1) - 1/2*(tempDW_IN1+tempDW_IN)/(deltaI^2);
    ...
    if discreteGradientCondition_II
        ...
        tempDW_IIN1 = ANNObject.computeEnergyANNCGJ(CN1,GN1,JN)-ANNObject.computeEnergyANNCGJ(CN1,GN,JN);
        tempDW_IIN = ANNObject.computeEnergyANNCGJ(CN,GN1,JN1)-ANNObject.computeEnergyANNCGJ(CN,GN,JN1);
...
%         D2W(2,2) = D2W(2,2) - 1/2*(tempDW_IIN1+tempDW_IIN)/(deltaII^2);        
    ...
    if discreteGradientCondition_J
        ...
        tempDW_JN1 = ANNObject.computeEnergyANNCGJ(CN1,GN1,JN1)-ANNObject.computeEnergyANNCGJ(CN1,GN1,JN);
        tempDW_JN = ANNObject.computeEnergyANNCGJ(CN,GN,JN1)-ANNObject.computeEnergyANNCGJ(CN,GN,JN);
        DW_J = 1/2*(tempDW_JN1+tempDW_JN)/deltaJ;
...
%         D2W(3,1:3) = 1/2*(tempD2W_cN1+tempD2W_cN)/deltaJ;
%         D2W(3,3) = D2W(3,3) - 1/2*(tempDW_JN1+tempDW_JN)/(deltaJ^2);        
    else
    ...
    SN05 = 2*(DW_I*dIC_C + wedge(DW_II*dIIC_G, CAlgo) + DW_J * 1/2 * JAlgo^(-1) * 1 / 3 * (wedge(CAlgo, CAlgo) + GAlgo1));
    ...
    if ~computePostData
        ...
        if 0
            ...
            KmatcgeoN1 = DW_J * 2 / 3 *(secDiffOperator(CAlgo) + 0.5*secDiffOperator(CN1));
        ...
        % Strain energy
        WN1 = ANNObject.computeEnergyANNCGJ(CN1,GN1,JN1);
