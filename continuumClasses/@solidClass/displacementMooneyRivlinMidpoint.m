function [rData, kData, elementEnergy, array] = displacementMooneyRivlinMidpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
%         DPsiComDCxN05 = (a+b*trace(CxN05))*eye(3) - b*CxN05 + ftilde(JN05, lambda, a, b) * CxN05inv;
...
%         DDPsiComDCxN05DCxN05V = b*((unityV*unityV.') - Isym) - ftilde(JN05, lambda, a, b)*sym_voigt(CxN05inv,CxN05inv,'stress','stress') + g(JN05, lambda)*(CxN05invV*CxN05invV.');
%         DDPsiComDCxN05DCxN05V = (mu/2 - auxilaryFunctionDW(JN05, lambda))*sym_voigt(CxN05inv,CxN05inv,'stress','stress') + auxilaryFunctionDW2(JN05, lambda)*(CxN05invV*CxN05invV.');
        ...
        psiComN1 = internalEnergy(CxN1, CxN1inv, JN1, lambda, a, b);
        psiComN = internalEnergy(CxN, CxNinv, JN, lambda, a, b);
...
function [energy] = internalEnergy(C, Cinv, J, lambda, a, b)
energy = a*(trace(C) - 3 - 2*log(J)) + b*(trace(J^2*Cinv) - 3 - 4*log(J)) + internalEnergyVolumetricPart(J, lambda);
end
