function [rData, kData, elementEnergy, array] = mixedSCpHGJLambdaMooneyRivlinVol2DiscreteGradient(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    if discreteGradientCondition_J
        ...
        DW_J = (c/2*(JN1-1)^2 - d*(JN1-1) - (c/2*(JN-1)^2 - d*(JN-1)))/deltaJ;
    else
    ...
    if ~computePostData
        ...
        % Strain energy
        W = a * (trace(CxN1) - 3) + b * (trace(GN1) - 3) + c / 2 * (JN1 - 1)^2 - d * (JN1-1);
