function [rData, kData, elementEnergy, array] = displacementNeoHookeDiscreteGradient(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)

%% Creates the residual and the tangent of the given obj.
%
% Description:
%   material model: Neo Hooke in lambda and mu 
%   description in S and C
%   Integrator: Discrete Gradient
%
% 08.08.2023 Moritz Hille
...
for k = 1:numberOfGausspoints
    ...
    %% residual, energy and tangent
        ...
        % energy
        psiComN1 = internalEnergy(CxN1, JN1, mu, lambda);
        psiComN = internalEnergy(CxN, JN, mu, lambda);
        ...
        if normDeltaCx < 100*eps % case midpoint rule 
            ...
            DPsiComDCxN05 = firstDerivativeInternalEnergy(CxN05inv, JN05, mu, lambda);
            DDPsiComDCxN05DCxN05V = secondDerivativeInternalEnergy(CxN05inv, JN05, mu, lambda);
            ...
            % first derivatives of internal energy: e(C) = psiCom(C)
            DPsiComDCxN1 = firstDerivativeInternalEnergy(CxN1inv, JN1, mu, lambda);
...
%% auxiliary functions
function [psi] = internalEnergy(C, J, mu, lambda)
psi = mu / 2 * (trace(C) - 3 - 2 * log(J)) + lambda / 2 * (log(J)^2 + (J - 1)^2);
end
...
function [DPsi] = firstDerivativeInternalEnergy(Cinv, J, mu, lambda)
DPsi = mu / 2 * (eye(3) - Cinv) + auxilaryFunctionDW(J, lambda) * Cinv;
end

function [DDPsi] = secondDerivativeInternalEnergy(Cinv, J, mu, lambda)
CinvV = matrixToVoigt(Cinv, 'stress');
DDPsi = (mu/2 - auxilaryFunctionDW(J, lambda))*sym_voigt(Cinv,Cinv,'stress','stress') + auxilaryFunctionDW2(J, lambda)*(CinvV*CinvV.');
end
