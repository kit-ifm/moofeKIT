function [rData, kData, elementEnergy, array] = mixedPHEHyperelasticDiscreteGradient(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
% MIXEDPHEHYPERELASTICDISCRETEGRADIENT Element routine of class solidClass.
%
% FORMULATION
...
% strains.
% The routine is suitable for dynamic simulation where the discrete 
% gradient is applied.
%
% CALL
...
%
% SEE ALSO
% mixedPHEHookeMidpoint, mixedPHEHyperelasticMidpoint
%
% CREATOR(S)
% Philipp Kinon
...
if ~computePostData
    % residual
    WN = getStrainEnergy(C, epsN);
    WN1 = getStrainEnergy(C, epsN1);
    Rv = Kveps * (WN1 - WN)/(epsN1-epsN);
    if abs(epsN1 - epsN) < 1e-7
        Rv = Kveps * sigmaN05;
    end
    ...
    % tangent
    kData{1,1} = zeros(2,2);
    kData{1,2} = Kveps * (getStress(C,epsN1)/(epsN1 -epsN)-(WN1 - WN)/(epsN1-epsN)^2);
    if abs(epsN1 - epsN) < 1e-7
