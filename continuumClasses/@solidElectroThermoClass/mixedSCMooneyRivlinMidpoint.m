...
% MIXEDSCMOONEYRIVLINMIDPOINT Element routine of class solidElectroThermoClass.
%
% FORMULATION
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
        ...
        DW_G = b * eye(3);
        DW_c = -d1 / (2 * cN05) + c1 / 2 * (1 - cN05^(-1 / 2)) - 1 / (4 * er * e0 * cN05^(3 / 2)) * DN05' * (CN05 * DN05) - dimension * beta * c2 * (thetaN05e-thetaR);

        %% Constitutive equations
        etaN05 = dimension * beta * c2 * (cN05 - 1) + kappa * log(thetaN05e/thetaR);
        QN05 = -k0 * cN05^(-1) * GN05 * (dNx * thetaN05');
        
        %% Residual
        RX = RX + BN05' * 2 * matrixToVoigt(lambdaCN1, 'stress') * detJ * gaussWeight(k);
        RT = RT + (N(k, :)' * (thetaN1e * etaN1 - thetaNe * etaN) / DT + N(k, :)' * (thetaN1e-thetaNe) / DT * DW_theta - dNx' * QN05) * detJ * gaussWeight(k);
        RP = RP + (dNx' * DN05) * detJ * gaussWeight(k);
        RD = RD + kron(M(k, :)', (DW_D + dNx * phiN05')) * detJ * gaussWeight(k);
        ...
        RLambdaCv = RLambdaCv + kron(M(k, :)', matrixToVoigt(FxN05'*FxN05-CN05, 'strain')) * detJ * gaussWeight(k);
        RLambdaGv = RLambdaGv + kron(M(k, :)', matrixToVoigt(0.5*wedge(CN05, CN05)-GN05, 'strain')) * detJ * gaussWeight(k);
        RLambdac = RLambdac + M(k, :)' * (innerProduct(1/3*GN05, CN05) - cN05) * detJ * gaussWeight(k);

        %% Tangent
        % KXX
        geometricalTangentPart = 1 / 2 * dNx' * 2 * lambdaCN1 * dNx;
        ...
        % KXLambdaC
        KXLambdaC = KXLambdaC + kron(M(k, :), 2*BN05') * detJ * gaussWeight(k);

        % =====

        % KTT
        dEtaN1_thetaN1 = kappa / thetaN1e;
        dEtaN05_thetaN05 = kappa / thetaN05e;
        kTT1 = N(k, :)' * (etaN1 + thetaN1e * dEtaN1_thetaN1) / DT * N(k, :);
        kTT2 = -N(k, :)' * (etaN05 + (thetaN1e-thetaNe) * dEtaN05_thetaN05 * 0.5) / DT * N(k, :);
        kTT3 = dNx' * k0 * cN05^(-1) * GN05 * 0.5 * dNx;
        KTT = KTT + (kTT1 + kTT2 + kTT3) * detJ * gaussWeight(k);

        % KTG
        KTG = KTG + kron(M(k, :), dNx'*k0*1/cN05*BErs(dNx*thetaN05')*0.5) * detJ * gaussWeight(k);

        % KTc
        dEtaN1_cN1 = dimension * beta * c2;
        dEtaN05_cN05 = dimension * beta * c2;
        kTc1 = kron(N(k, :)'*M(k, :), (thetaN1e * dEtaN1_cN1 - (thetaN1e-thetaNe) * dEtaN05_cN05 * 0.5)/DT);
        kTc2 = -kron(M(k, :), dNx'*k0*1/(cN05^2)*GN05*(dNx * thetaN05')*0.5);
        KTc = KTc + (kTc1 + kTc2) * detJ * gaussWeight(k);

        % =====

        % KPD
        KPD = KPD + kron(M(k, :), dNx'*0.5) * detJ * gaussWeight(k);

        % =====

        % KDP
        KDP = KDP + kron(M(k, :)', 0.5*dNx) * detJ * gaussWeight(k);

        % KDD
        KDD = KDD + kron(M(k, :)'*M(k, :), 1/(er * e0 * sqrt(cN05))*CN05*0.5) * detJ * gaussWeight(k);

        % KDC
        KDC = KDC + kron(M(k, :)'*M(k, :), 1/(er * e0 * sqrt(cN05))*BErs(DN05)*0.5) * detJ * gaussWeight(k);

        % KDc
        D2W_D_cN05 = -1 / (2 * er * e0 * (cN05)^(3 / 2)) * CN05 * DN05;
        KDc = KDc + kron(M(k, :)'*M(k, :), D2W_D_cN05*0.5) * detJ * gaussWeight(k);

        % =====

        % KCD
        KCD = KCD + kron(M(k, :)'*M(k, :), 1/(2 * er * e0 * sqrt(cN05))*AErs(DN05)*0.5) * detJ * gaussWeight(k);

        % KCC
        KCC = KCC + 1 / 2 * kron(M(k, :)'*M(k, :), secDiffOperator(lambdaGN1)) * detJ * gaussWeight(k);

        % KCG
        KCG = KCG + 1 / 6 * kron(M(k, :)'*M(k, :), lambdacN1*map.Isym) * detJ * gaussWeight(k);

        % KCc
        D2W_C_cN05 = -1 / (4 * er * e0 * cN05^(3 / 2)) * matrixToVoigt(DN05*DN05', 'strain');
        KCc = KCc + kron(M(k, :)'*M(k, :), 1/2*D2W_C_cN05) * detJ * gaussWeight(k);

        % KCLambdaC
        KCLambdaC = KCLambdaC - kron(M(k, :)'*M(k, :), map.Isym) * detJ * gaussWeight(k);

        % KCLambdaG
        KCLambdaG = KCLambdaG + kron(M(k, :)'*M(k, :), secDiffOperator(CN05)) * detJ * gaussWeight(k);
        ...
        % KGC
        KGC = KGC + 1 / 6 * kron(M(k, :)'*M(k, :), lambdacN1*map.Isym) * detJ * gaussWeight(k);
        ...
        % KcD
        KcD = KcD - kron(M(k, :)'*M(k, :), 1/(2 * er * e0 * cN05^(3 / 2))*(CN05 * DN05)'*0.5) * detJ * gaussWeight(k);

        % KcT
        D2W_c_thetaN05 = -dimension * beta * c2;
        KcT = KcT + kron(M(k, :)'*N(k, :), D2W_c_thetaN05*0.5) * detJ * gaussWeight(k);

        % KcC
        KcC = KcC - kron(M(k, :)'*M(k, :), 1/(4 * er * e0 * cN05^(3 / 2))*DN05'*BErs(DN05)*0.5) * detJ * gaussWeight(k);

        % Kcc
        D2W_c_cN05 = d1 / (2 * cN05^2) + c1 / (4 * cN05^(3 / 2)) + 3 / (8 * er * e0 * cN05^(5 / 2)) * DN05' * (CN05 * DN05);
        Kcc = Kcc + kron(M(k, :)'*M(k, :), D2W_c_cN05*0.5) * detJ * gaussWeight(k);
        ...
        % KLambdaCC
        KLambdaCC = KLambdaCC - 1 / 2 * kron(M(k, :)'*M(k, :), map.Isym) * detJ * gaussWeight(k);

        % =====

        % KLambdaGC
        KLambdaGC = KLambdaGC + 1 / 2 * kron(M(k, :)'*M(k, :), secDiffOperator(CN05)) * detJ * gaussWeight(k);

        % KLambdaGG
        KLambdaGG = KLambdaGG - 1 / 2 * kron(M(k, :)'*M(k, :), map.Isym) * detJ * gaussWeight(k);
