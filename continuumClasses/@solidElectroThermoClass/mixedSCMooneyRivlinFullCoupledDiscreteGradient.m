function [rData, kData, elementEnergy, array] = mixedSCMooneyRivlinFullCoupledDiscreteGradient(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
% MIXEDSCMOONEYRIVLINFULLCOUPLEDDISCRETEGRADIENT Element routine of class solidElectroThermoClass.
%
% FORMULATION
...
% In contrast to routine mixedSCMooneyRivlinDiscreteGradient, this routine
% also considers thermoelectric coupling.
%
% CALL
% mixedSCMooneyRivlinFullCoupledDiscreteGradient(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
% mixedSCMooneyRivlinFullCoupledMidpoint
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
        ...
        if ~flagNumericalTangent
            ...
            discreteGradientCondition_G = flagDiscreteGradient(k, 4);
            discreteGradientCondition_c = flagDiscreteGradient(k, 5);
        end
        ...
        if discreteGradientCondition_T
            ...
            DW_theta = 1 / 2 * (fThetaN1 - fThetaN) / deltaT * (a * (trace(CN1) - 3) + b * (trace(GN1) - 3) + c1 / 2 * (sqrt(cN1) - 1)^2 - d1 * log(sqrt(cN1)) + 1 / (2 * er * e0 * sqrt(cN1)) * DN1.' * CN1 * DN1) ...
                +1 / 2 * (fThetaN1 - fThetaN) / deltaT * (a * (trace(CN) - 3) + b * (trace(GN) - 3) + c1 / 2 * (sqrt(cN) - 1)^2 - d1 * log(sqrt(cN)) + 1 / (2 * er * e0 * sqrt(cN)) * DN.' * CN * DN) ...
                -3 / 2 * beta * c2 * (cN1 + cN - 2) + kappa * (1 - thetaN1e / deltaT * log(thetaN1e) + thetaNe / deltaT * log(thetaNe) + log(thetaR));
        else
...
%         if discreteGradientCondition_D
        if setupObject.newton.step > 1
            ...
            DW_D = fThetaN * 1 / (2 * er * e0 * sqrt(cN1)) * (CN1 * DN05) + fThetaN1 * 1 / (2 * er * e0 * sqrt(cN)) * (CN * DN05);
        else
        ...
        if discreteGradientCondition_C
            ...
            DW_C = 1 / 2 * (fThetaN * a * eye(3) + fThetaN1 * a * eye(3)) + 1 / (4 * er * e0) * (fThetaN * 1 / sqrt(cN) * (DN * DN.') + fThetaN1 * 1 / sqrt(cN1) * (DN1 * DN1.'));
        else
        ...
        if discreteGradientCondition_G
            ...
            DW_G = 1 / 2 * (fThetaN * b * eye(3) + fThetaN1 * b * eye(3));
        else
        ...
        if discreteGradientCondition_c
            flagDiscreteGradient(k, 5) = 1;
            DW_c = fThetaN05 / deltac * (c1 / 2 * ((sqrt(cN1) - 1)^2 - (sqrt(cN) - 1)^2) + d1 * (log(sqrt(cN)) - log(sqrt(cN1)))) ...
                +1 / (4 * er * e0 * deltac) * (1 / sqrt(cN1) - 1 / sqrt(cN)) * (fThetaN * DN.' * (CN1 * DN) + fThetaN1 * DN1.' * (CN * DN1)) ...
                -dimension * beta * c2 * (thetaN05e-thetaR);
        else
            flagDiscreteGradient(k, 5) = 0;
        ...
        % KTT
        kTT1 = N_k_I(k, :)' * N_k_I(k, :) * 1 / DT * (etaN1 + DW_theta);
        ...
        if flagDiscreteGradient(k, 1)
            kTD2 = 1 / 2 * kron(N_k_I(k, :)'*M_k_I(k, :), 1/DT*(thetaN1e-thetaNe)*DfThetaN1_theta*1/(er * e0 * sqrt(cN1))*(CN1 * DN1)');
        else
            kTD2 = 1 / 2 * kron(N_k_I(k, :)'*M_k_I(k, :), 1/DT*(thetaN1e-thetaNe)*DfThetaN05_theta*1/(er * e0 * sqrt(cN05))*(CN05 * DN05)');
        end
        ...
        if flagDiscreteGradient(k, 1)
            D2W_theta_CN1 = 1 / 2 * DfThetaN1_theta * (a * matrixToVoigt(eye(3), 'strain')' + 1 / (2 * er * e0 * sqrt(cN1)) * DN1' * BErs(DN1));
            kTC1 = kron(N_k_I(k, :)'*M_k_I(k, :), 1/DT*(thetaN1e * dEtaN1_CN1 + (thetaN1e-thetaNe) * D2W_theta_CN1));
        else
        ...
        % KTG
        dEtaN1_GN1 = -DfThetaN1_theta * b * matrixToVoigt(eye(3), 'strain')';
        D2W_theta_GN1 = 1 / 2 * DfThetaN1_theta * b * matrixToVoigt(eye(3), 'strain')';
        kTG1 = kron(N_k_I(k, :)'*M_k_I(k, :), (thetaN1e * dEtaN1_GN1 + (thetaN1e-thetaNe) * D2W_theta_GN1)/DT);
        kTG2 = 1 / 2 * kron(M_k_I(k, :), dN_X_I'*k0/cN05*BErs(dN_X_I*thetaN05'));
        ...
        if flagDiscreteGradient(k, 1)
            D2W_theta_cN1 = 1 / 2 * (1 / thetaR * (c1 / 2 * (1 - 1 / sqrt(cN1)) - d1 / (2 * cN1) - 1 / (4 * er * e0 * (cN1)^(3 / 2)) * DN1' * (CN1 * DN1)) - dimension * beta * c2);
        else
        ...
        if flagDiscreteGradient(k, 2)
            kDT1 = 1 / 2 * kron(M_k_I(k, :)'*N_k_I(k, :), DfThetaN1_theta*1/(er * e0 * sqrt(cN))*CN*DN05);
        else
            kDT1 = 1 / 2 * kron(M_k_I(k, :)'*N_k_I(k, :), DfThetaN05_theta*1/(er * e0 * sqrt(cN05))*CN05*DN05);
        end
        KDT = KDT + kDT1 * detJ * gaussWeight(k);
        ...
        % KDD
        if flagDiscreteGradient(k, 2)
            D2W_D_DN1 = 1 / 4 * (fThetaN * 1 / (er * e0 * sqrt(cN1)) * CN1 + fThetaN1 * 1 / (er * e0 * sqrt(cN)) * CN);
            kDD1 = kron(M_k_I(k, :)'*M_k_I(k, :), D2W_D_DN1);
        else
            kDD1 = 1 / 2 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN05*1/(er * e0 * sqrt(cN05))*CN05);
        end
        KDD = KDD + kDD1 * detJ * gaussWeight(k);

        % KDC
        if flagDiscreteGradient(k, 2)
            kDC1 = 1 / 2 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN*1/(er * e0 * sqrt(cN1))*BErs(DN05));
        else
            kDC1 = 1 / 2 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN05*1/(er * e0 * sqrt(cN05))*BErs(DN05));
        end
        KDC = KDC + kDC1 * detJ * gaussWeight(k);

        % KDc
        if flagDiscreteGradient(k, 2)
            D2W_D_cN1 = -1 / 4 * fThetaN * 1 / (er * e0 * (cN1)^(3 / 2)) * CN1 * DN05;
            KDc1 = kron(M_k_I(k, :)'*M_k_I(k, :), D2W_D_cN1);
        else

            KDc1 = -1 / 4 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN05*1/(er * e0 * (cN05)^(3 / 2))*CN05*DN05);
        end
        KDc = KDc + KDc1 * detJ * gaussWeight(k);
        ...
        if flagDiscreteGradient(k, 3)
            kCT1 = 1 / 2 * kron(M_k_I(k, :)'*N_k_I(k, :), DfThetaN1_theta*matrixToVoigt(a*eye(3)+1/(2 * er * e0 * sqrt(cN1))*(DN1 * DN1'), 'strain'));
        else
            kCT1 = 1 / 2 * kron(M_k_I(k, :)'*N_k_I(k, :), DfThetaN05_theta*matrixToVoigt(a*eye(3)+1/(2 * er * e0 * sqrt(cN05))*(DN05 * DN05'), 'strain'));
        end
        KCT = KCT + kCT1 * detJ * gaussWeight(k);

        % KCD
        if flagDiscreteGradient(k, 3)
            kCD1 = 1 / 4 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN1*1/(er * e0 * sqrt(cN1))*AErs(DN1));
        else
            kCD1 = 1 / 4 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN05*1/(er * e0 * sqrt(cN05))*AErs(DN05));
        end
        ...
        % KCc
        if flagDiscreteGradient(k, 3)
            KCc1 = -1 / 8 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN1*1/(er * e0 * (cN1)^(3 / 2))*matrixToVoigt(DN1*DN1', 'strain'));
        else
            KCc1 = -1 / 8 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN05*1/(er * e0 * (cN05)^(3 / 2))*matrixToVoigt(DN05*DN05', 'strain'));
        end
        ...
        % KcT
        if flagDiscreteGradient(k, 5)
            D2W_c_thetaN1 = 1 / 2 * DfThetaN05_theta / deltac * (c1 / 2 * ((sqrt(cN1) - 1)^2 - (sqrt(cN) - 1)^2) + d1 * log(sqrt(cN/cN1))) + 1 / (4 * er * e0 * deltac) * (1 / sqrt(cN1) - 1 / sqrt(cN)) * 1 / thetaR * DN1' * CN * DN1;
            kcT1 = kron(M_k_I(k, :)'*N_k_I(k, :), D2W_c_thetaN1);
        else
            D2W_c_thetaN1 = 1 / 2 * DfThetaN05_theta * (c1 / 2 * (1 - 1 / sqrt(cN05)) - d1 / (2 * cN05) - 1 / (4 * er * e0 * cN05^(3 / 2)) * DN05' * CN05 * DN05);
            kcT1 = kron(M_k_I(k, :)'*N_k_I(k, :), D2W_c_thetaN1);
        end
        kcT2 = -1 / 2 * kron(M_k_I(k, :)'*N_k_I(k, :), dimension*beta*c2);
        KcT = KcT + (kcT1 + kcT2) * detJ * gaussWeight(k);

        % KcD
        if flagDiscreteGradient(k, 5)
            KcD1 = 1 / 2 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN1*1/(er * e0 * deltac)*(1 / sqrt(cN1) - 1 / sqrt(cN))*(CN * DN1)');
        else
            KcD1 = -1 / 4 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN05*1/(er * e0 * cN05^(3 / 2))*(CN05 * DN05)');
        end
        KcD = KcD + KcD1 * detJ * gaussWeight(k);

        % KcC
        if flagDiscreteGradient(k, 5)
            KcC1 = 1 / 4 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN*1/(er * e0 * deltac)*(1 / sqrt(cN1) - 1 / sqrt(cN))*DN'*BErs(DN));
        else
            KcC1 = -1 / 8 * kron(M_k_I(k, :)'*M_k_I(k, :), fThetaN05*1/(er * e0 * cN05^(3 / 2))*DN05'*BErs(DN05));
        end
        KcC = KcC + KcC1 * detJ * gaussWeight(k);

        % Kcc
        if flagDiscreteGradient(k, 5)
            D2W_c_cN1 = -1 / 2 * fThetaN05 * d1 / (cN1 * deltac) ...
                -fThetaN05 * d1 / (deltac)^2 * log(sqrt(cN/cN1)) ...
                +1 / 2 * fThetaN05 * c1 / deltac * (1 - 1 / sqrt(cN1)) ...
                -1 / 2 * fThetaN05 * c1 / (deltac)^2 * ((sqrt(cN1) - 1)^2 - (sqrt(cN) - 1)^2) ...
                -1 / 8 * 1 / (er * e0 * (cN1)^(3 / 2) * deltac) * (fThetaN * DN' * (CN1 * DN) + fThetaN1 * DN1' * (CN * DN1)) ...
                -1 / 4 * 1 / (er * e0 * deltac^2) * (1 / sqrt(cN1) - 1 / sqrt(cN)) * (fThetaN * DN' * (CN1 * DN) + fThetaN1 * DN1' * (CN * DN1));

            Kcc1 = M_k_I(k, :)' * M_k_I(k, :) * D2W_c_cN1;
        else
            D2W_c_cN1 = fThetaN05 * (d1 / (4 * cN05^2) + c1 / (8 * cN05^(3 / 2)) + 3 / (16 * er * e0 * cN05^(5 / 2)) * DN05' * (CN05 * DN05));
            Kcc1 = M_k_I(k, :)' * M_k_I(k, :) * D2W_c_cN1;
        end
        Kcc = Kcc + Kcc1 * detJ * gaussWeight(k);
...
DW_theta = 1 / thetaR * (a * (trace(C) - 3) + b * (trace(G) - 3) + c1 / 2 * (sqrt(c) - 1)^2 - d1 * log(sqrt(c)) + 1 / (2 * er * e0 * sqrt(c)) * D.' * (C * D)) - 3 * beta * c2 * (c - 1) - kappa * (log(theta) - log(thetaR));
end
