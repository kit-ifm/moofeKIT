function out = mmhwGenericCascadeSC_mooneyRivlin_endPointNumTang(obj,varargin)
...
numNodes_c = size(N_c,2);
numberOfInternalDOFs = (numNodes_C+numNodes_G)*6+numNodes_c;
DIM = obj.DIM;
...
parfor j = 1:numElements
    ...
    [RX,REPS,RSIG,RT,KXX,KXSIG,KEPSEPS,KEPSSIG,KEPST,KSIGX,KSIGEPS,KTEPS,KTT,ePot,flagDiscreteGradient] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1,thetaN,QREF,CN1e,GN1e,cN1e,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,0);
...
%             [RXxx,~,~,~,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1xx,thetaN1,thetaN,QREF,CN1e,GN1e,cN1e,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
%             KXXxx(:,xx) = (RXxx - RX)/epsilon;
%         end
%         KXX = KXXxx;

%         KXSIGxx = zeros(numberOfMechanicalDOFs,numberOfInternalDOFs);
%         for xx = 1:1:numberOfInternalDOFs
%             SIGMA_CN1exx = SIGMA_CN1e;
%             SIGMA_GN1exx = SIGMA_GN1e;
%             SIGMA_cN1exx = SIGMA_cN1e;
%             SIGMA_CN1xx = [SIGMA_CN1e(j,:)';SIGMA_GN1e(j,:)';SIGMA_cN1e(j,:)'];
%             SIGMA_CN1xx(xx) = SIGMA_CN1xx(xx)+epsilon;
%             SIGMA_CN1exx(j,:) = SIGMA_CN1xx(1:numNodes_C*6);
%             SIGMA_GN1exx(j,:) = SIGMA_CN1xx(numNodes_C*6+1:(numNodes_C+numNodes_G)*6);
%             SIGMA_cN1exx(j,:) = SIGMA_CN1xx((numNodes_C+numNodes_G)*6+1:numberOfInternalDOFs);
%             [RXxx,~,~,~,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1,thetaN,QREF,CN1e,GN1e,cN1e,SIGMA_CN1exx,SIGMA_GN1exx,SIGMA_cN1exx,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
%             KXSIGxx(:,xx) = (RXxx - RX)/epsilon;
%         end
%         KXSIG = KXSIGxx;
        
%         KEPSEPSxx = zeros(numberOfInternalDOFs,numberOfInternalDOFs);
%         for xx = 1:1:numberOfInternalDOFs
%             CN1exx = CN1e;
%             GN1exx = GN1e;
%             cN1exx = cN1e;
%             CN1xx = [CN1e(j,:)';GN1e(j,:)';cN1e(j,:)'];
%             CN1xx(xx) = CN1xx(xx)+epsilon;
%             CN1exx(j,:) = CN1xx(1:numNodes_C*6);
%             GN1exx(j,:) = CN1xx(numNodes_C*6+1:(numNodes_C+numNodes_G)*6);
%             cN1exx(j,:) = CN1xx((numNodes_C+numNodes_G)*6+1:numberOfInternalDOFs);
%             [~,REPSxx,~,~,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1,thetaN,QREF,CN1exx,GN1exx,cN1exx,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
%             KEPSEPSxx(:,xx) = (REPSxx - REPS)/epsilon;
%         end
%         KEPSEPS = KEPSEPSxx;
        
%         KEPSSIGxx = zeros(numberOfInternalDOFs,numberOfInternalDOFs);
%         for xx = 1:1:numberOfInternalDOFs
%             SIGMA_CN1exx = SIGMA_CN1e;
%             SIGMA_GN1exx = SIGMA_GN1e;
%             SIGMA_cN1exx = SIGMA_cN1e;
...
%             SIGMA_GN1exx(j,:) = SIGMA_CN1xx(numNodes_C*6+1:(numNodes_C+numNodes_G)*6);
%             SIGMA_cN1exx(j,:) = SIGMA_CN1xx((numNodes_C+numNodes_G)*6+1:numberOfInternalDOFs);
%             [~,REPSxx,~,~,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1,thetaN,QREF,CN1e,GN1e,cN1e,SIGMA_CN1exx,SIGMA_GN1exx,SIGMA_cN1exx,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
%             KEPSSIGxx(:,xx) = (REPSxx - REPS)/epsilon;
%         end
%         KEPSSIG = KEPSSIGxx;
%         
%         KEPSTxx = zeros(numberOfInternalDOFs,numberOfTemperatureDOFs);
%         for xx = 1:1:numberOfTemperatureDOFs
%             thetaN1xx = thetaN1;
%             thetaN1xx(xx) = thetaN1xx(xx)+epsilon;
%             [~,REPSxx,~,~,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1xx,thetaN,QREF,CN1e,GN1e,cN1e,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
%             KEPSTxx(:,xx) = (REPSxx - REPS)/epsilon;
%         end
%         KEPST = KEPSTxx;

%         KSIGXxx = zeros(numberOfInternalDOFs,numberOfMechanicalDOFs);
%         for xx = 1:1:numberOfMechanicalDOFs
%             edN1xx = edN1;
%             edN1xx(xx) = edN1xx(xx)+epsilon;
%             [~,~,RSIGxx,~,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1xx,thetaN1,thetaN,QREF,CN1e,GN1e,cN1e,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
%             KSIGXxx(:,xx) = (RSIGxx - RSIG)/epsilon;
%         end
%         KSIGX = KSIGXxx;

%         KSIGEPSxx = zeros(numberOfInternalDOFs,numberOfInternalDOFs);
%         for xx = 1:1:numberOfInternalDOFs
%             CN1exx = CN1e;
%             GN1exx = GN1e;
%             cN1exx = cN1e;
...
%             GN1exx(j,:) = CN1xx(numNodes_C*6+1:(numNodes_C+numNodes_G)*6);
%             cN1exx(j,:) = CN1xx((numNodes_C+numNodes_G)*6+1:numberOfInternalDOFs);
%             [~,~,RSIGxx,~,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1,thetaN,QREF,CN1exx,GN1exx,cN1exx,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
%             KSIGEPSxx(:,xx) = (RSIGxx - RSIG)/epsilon;
%         end
%         KSIGEPS = KSIGEPSxx;

%         KTEPSxx = zeros(numberOfTemperatureDOFs,numberOfInternalDOFs);
%         for xx = 1:1:numberOfInternalDOFs
%             CN1exx = CN1e;
%             GN1exx = GN1e;
%             cN1exx = cN1e;
%             CN1_xx = [CN1e(j,:)';GN1e(j,:)';cN1e(j,:)'];
%             CN1_xx(xx) = CN1_xx(xx)+epsilon;
%             CN1exx(j,:) = CN1_xx(1:numNodes_C*6);
%             GN1exx(j,:) = CN1_xx(numNodes_C*6+1:(numNodes_C+numNodes_G)*6);
%             cN1exx(j,:) = CN1_xx((numNodes_C+numNodes_G)*6+1:numberOfInternalDOFs);
%             [~,~,~,RTxx,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1,thetaN,QREF,CN1exx,GN1exx,cN1exx,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
%             KTEPSxx(:,xx) = (RTxx - RT)/epsilon;
%         end
%         KTEPS = KTEPSxx;
...
%             thetaN1xx = thetaN1;
%             thetaN1xx(xx) = thetaN1xx(xx) + epsilon;
%             [~,~,~,RTxx,~,~,~,~,~,~,~,~,~,~,~] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1xx,thetaN,QREF,CN1e,GN1e,cN1e,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent);
...
function [RX,REPS,RSIG,RT,KXX,KXSIG,KEPSEPS,KEPSSIG,KEPST,KSIGX,KSIGEPS,KTEPS,KTT,ePot,flagDiscreteGradient] = Residuum(DT,j,numDOFs,edN,edN1,thetaN1,thetaN,QREF,CN1e,GN1e,cN1e,SIGMA_CN1e,SIGMA_GN1e,SIGMA_cN1e,cNe,CNe,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent)
...
% Number of internal dofs
N_C = objNT.SHAPEF_F.N;
N_G = objNT.SHAPEF_Cof.N;
N_c =  objNT.SHAPEF_Det.N;
numNodes_C = size(N_C,2);
numNodes_G = size(N_G,2);
numNodes_c = size(N_c,2);
...
for k = 1:NGP
    ...
    u0N1 = PsiIsoN1 + PsiVolN1 + uThermN1 + uCoupledN1;
%     
