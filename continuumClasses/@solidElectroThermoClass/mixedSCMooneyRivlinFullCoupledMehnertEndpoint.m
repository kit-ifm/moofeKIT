function [rData, kData, elementEnergy, array] = mixedSCMooneyRivlinFullCoupledMehnertEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, ~)
% MIXEDSCMOONEYRIVLINFULLCOUPLEDMEHNERTENDPOINT Element routine of class solidElectroThermoClass.
%
% FORMULATION
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
        ...
        %% energy
        factorMehnertA = 69.84;
        factorMehnertB = 0.196;
        fThetaN = thetaNe / thetaR - thetaNe * (tanh(factorMehnertB*(thetaNe - thetaR)))^3 / (thetaR + factorMehnertA);
        fThetaN1 = thetaN1e / thetaR - thetaN1e * (tanh(factorMehnertB*(thetaN1e-thetaR)))^3 / (thetaR + factorMehnertA);
        DfThetaN_theta = 1 / thetaR - 1 / (thetaR + factorMehnertA) * ((tanh(factorMehnertB*(thetaNe - thetaR)))^3 + thetaNe * 3 * factorMehnertB * (tanh(factorMehnertB*(thetaNe - thetaR)))^2 * (1 - (tanh(factorMehnertB*(thetaNe - thetaR)))^2));
        DfThetaN1_theta = 1 / thetaR - 1 / (thetaR + factorMehnertA) * ((tanh(factorMehnertB*(thetaN1e-thetaR)))^3 + thetaN1e * 3 * factorMehnertB * (tanh(factorMehnertB*(thetaN1e-thetaR)))^2 * (1 - (tanh(factorMehnertB*(thetaN1e-thetaR)))^2));
        D2fThetaN1_theta_theta = - 6 * factorMehnertB / (thetaR + factorMehnertA) * (tanh(factorMehnertB*(thetaN1e - thetaR)))^2 * (1 - (tanh(factorMehnertB*(thetaN1e - thetaR)))^2) - 6 * thetaN1e / (thetaR + factorMehnertA) * tanh(factorMehnertB*(thetaN1e-thetaR)) * (1 - (tanh(factorMehnertB*(thetaN1e-thetaR)))^2)^2 * factorMehnertB^2 + 6 * thetaN1e / (thetaR + factorMehnertA) * (tanh(factorMehnertB*(thetaN1e - thetaR)))^2 * tanh(factorMehnertB*(thetaN1e - thetaR)) * (1 - (tanh(factorMehnertB*(thetaN1e-thetaR)))^2) * factorMehnertB^2;
        ...
        % KTT
        D2W_theta_thetaN1 = -kappa / thetaN1e + D2fThetaN1_theta_theta * (a * (trace(CN1) - 3) + b * (trace(GN1) - 3) + c1 / 2 * (sqrt(cN1) - 1)^2 - d1 * log(sqrt(cN1)) + 1 / (2 * er * e0 * sqrt(cN1)) * DN1.' * (CN1 * DN1));
        dEtaN1_thetaN1 = -D2W_theta_thetaN1;
        kTT1 = N(k, :)' * (etaN1 + thetaN1e * dEtaN1_thetaN1) / DT * N(k, :);
        kTT2 = N(k, :)' * (DW_theta + (thetaN1e-thetaNe) * D2W_theta_thetaN1) / DT * N(k, :);
        kTT3 = dNx' * k0 * cN1^(-1) * GN1 * dNx;
        KTT = KTT + (kTT1 + kTT2 + kTT3) * detJ * gaussWeight(k);

        % KTD
        kTD1 = -kron(N(k, :)'*M(k, :), 1/DT*thetaN1e*DfThetaN1_theta*1/(er * e0 * sqrt(cN1))*(CN1 * DN1)');
        kTD2 = kron(N(k, :)'*M(k, :), DfThetaN1_theta*1/DT*(thetaN1e-thetaNe)*1/(er * e0 * sqrt(cN1))*(CN1 * DN1)');
        ...
        kTC1 = kron(N(k, :)'*M(k, :), (-thetaN1e * D2W_theta_CN1 + (thetaN1e-thetaNe) * D2W_theta_CN1)/DT);
        KTC = KTC + (kTC1) * detJ * gaussWeight(k);

        % KTG
        D2W_theta_GN1 = matrixToVoigt(DfThetaN1_theta*b*eye(3), 'strain')';
        kTG1 = kron(N(k, :)'*M(k, :), (-thetaN1e * D2W_theta_GN1 + (thetaN1e-thetaNe) * D2W_theta_GN1)/DT);
        kTG2 = kron(M(k, :), dNx'*k0*1/cN1*BErs(dNx*thetaN1'));
        ...
        % KDT
        KDT = KDT + kron(M(k, :)'*N(k, :), DfThetaN1_theta*1/(er * e0 * sqrt(cN1))*CN1*DN1) * detJ * gaussWeight(k);

        % KDP
        KDP = KDP + kron(M(k, :)', dNx) * detJ * gaussWeight(k);

        % KDD
        KDD = KDD + kron(M(k, :)'*M(k, :), fThetaN1*1/(er * e0 * sqrt(cN1))*CN1) * detJ * gaussWeight(k);

        % KDC
        KDC = KDC + kron(M(k, :)'*M(k, :), fThetaN1*1/(er * e0 * sqrt(cN1))*BErs(DN1)) * detJ * gaussWeight(k);
        ...
        % KCT
        KCT = KCT + kron(M(k, :)'*N(k, :), DfThetaN1_theta*matrixToVoigt(a*eye(3)+1/(2 * er * e0 * sqrt(cN1))*(DN1 * DN1'), 'strain')) * detJ * gaussWeight(k);

        % KCD
        KCD = KCD + kron(M(k, :)'*M(k, :), fThetaN1*1/(2 * er * e0 * sqrt(cN1))*AErs(DN1)) * detJ * gaussWeight(k);
        ...
        % KGT
        KGT = KGT + kron(M(k, :)'*N(k, :), DfThetaN1_theta*b*matrixToVoigt(eye(3), 'strain')) * detJ * gaussWeight(k);
        ...
        % KcD
        KcD = KcD - kron(M(k, :)'*M(k, :), fThetaN1*1/(2 * er * e0 * cN1^(3 / 2))*(CN1 * DN1)') * detJ * gaussWeight(k);

        % KcC
        KcC = KcC - kron(M(k, :)'*M(k, :), fThetaN1*1/(4 * er * e0 * cN1^(3 / 2))*DN1'*BErs(DN1)) * detJ * gaussWeight(k);
