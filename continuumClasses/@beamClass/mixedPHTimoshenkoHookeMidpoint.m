...
% stress resultants
shear_forceN05 = shearCorFact*G*A*gammaN05;
bending_momentN05 = E*I*kappaN05;
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
        ...
        %residual (linear)
        rData{1} = rData{1} + K'*shear_forceN05 * detJ * gaussWeight(k);
        rData{2} = rData{2} + (K'*bending_momentN05 - L*shear_forceN05) * detJ * gaussWeight(k);
        ...
        %tangent (constant)
        kData{1,3} = kData{1,3} + 1/2*K'*shearCorFact*G*A * detJ * gaussWeight(k);
        kData{2,3} = kData{2,3} - 1/2*L *shearCorFact*G*A * detJ * gaussWeight(k);
        kData{2,4} = kData{2,4} + 1/2*K'*E*I * detJ * gaussWeight(k);
