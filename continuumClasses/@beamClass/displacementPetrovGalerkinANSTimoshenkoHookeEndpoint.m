function [rData, kData, elementEnergy, array] = displacementPetrovGalerkinANSTimoshenkoHookeEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, ~, ~)
% DISPLACEMENTPETROVGALERKINANSTIMOSHENKOENDPOINT Element routine of class beamClass.
%
% FORMULATION
...
% More precisely, the transverse shear is approximated via ANS, while the
% virtual transverse shear is approximated in the usual way (Lagrange).
% Therefore it is a Petrov-Galerkin formulation.
%
% CALL
...
for k = 1:numberOfGausspoints
    ...
    L_bending = [0, dN_X_I(1), 0, dN_X_I(2)];

    B_shear = [dN_X_I(1), N_k_I(k, 1), dN_X_I(2), N_k_I(k, 2)];
    L_shear = [-1 / he, 1 / 2, 1 / he, 1 / 2];

    if ~computePostData

        %stiffness matrix
        stiffness_mat_shear = stiffness_mat_shear + shearCorFact * G * A * (B_shear') * L_shear * detJ * gaussWeight(k);
        stiffness_mat_bending = stiffness_mat_bending + E * I * (B_bending') * L_bending * detJ * gaussWeight(k);
        ...
        % strain energy
        kappaN1 = L_bending * uN1; %curvature
        gammaN1 = L_shear * uN1; % shear deformation
