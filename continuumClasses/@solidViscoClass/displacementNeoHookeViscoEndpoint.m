function [rData, kData, elementEnergy, array] = displacementNeoHookeViscoEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
    ...
    if ~computePostData
        ...
        while setupObject.newtonVisco.step <= setupObject.newtonVisco.maximumSteps
            ...
            for ii = 0:dimensionVisco
                ...
                DPsiVisDCiN1 = -0.5 * ((LambdaN1.') * DPsiVisDLambdaN1 * CiN1inv + CiN1inv * (DPsiVisDLambdaN1.') * LambdaN1);
                if ii == 0
                    DPsiVisDCiN1Resi = DPsiVisDCiN1;
                end
                % stresses
                GammaN1 = -DPsiVisDCiN1; % inelastic stress tensor
                MN1 = 2 * CiN1 * GammaN1; % Mandel stress tensor
        ...
        for ii = 0:dimR + dimensionVisco
            ...
            DPsiVisDLambdaN1 = muVisco / 2 * (eye(3) - eye(3) / (LambdaN1.')) + lambdaVisco / 2 * (log(JViscoN1) + JViscoN1^2 - JViscoN1) * (eye(3) / (LambdaN1.'));
            DPsiVisDCxN1 = 0.5 * (DPsiVisDLambdaN1 * CiN1inv + CiN1inv * DPsiVisDLambdaN1.');
            DPsiVisDCiN1 = -0.5 * ((LambdaN1.') * DPsiVisDLambdaN1 * CiN1inv + CiN1inv * (DPsiVisDLambdaN1.') * LambdaN1);
