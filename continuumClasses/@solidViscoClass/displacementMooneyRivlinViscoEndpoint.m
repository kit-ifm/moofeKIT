function [rData, kData, elementEnergy, array] = displacementMooneyRivlinViscoEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)

%% Creates the residual and the tangent of the given obj.
%
% Description:
%   material model: viscoelastic extention of Mooney-Rivlin  material model
%
% 24.04.2023 Marlon Franke
...
c = materialObject.c;
d = materialObject.d;
EVisco = materialObject.EVisco; % viscous Lamé-parameter lambda_e
...
for k = 1:numberOfGausspoints
    ...
    GammaN1V(:, :) = obj.epsilonViscoN(e, k, :);
    ...
    if ~computePostData
        ...
        while setupObject.newtonVisco.step <= setupObject.newtonVisco.maximumSteps
            ...
            for ii = 0:dimensionVisco
                GammaN1Vakt = GammaN1V;
                % inelastic Cauchy Green
                if ii > 0
                    GammaN1Vakt(ii) = GammaN1Vakt(ii) + numTangEps;
                end
                GammaN1 = voigtToMatrix(GammaN1Vakt, 'strain');
                GammaN = voigtToMatrix(GammaNV, 'strain');
                GammaN1inv = GammaN1 \ eye(dimension);

                % auxiliary variables
                LambdaN1 = CxN1 / GammaN1;
                JViscoN1 = sqrt(det(LambdaN1));

                % first derivatives of internal energy: (e = psiCom + psiVis)
                DPsiVisDLambdaN1 = muVisco / 2 * (eye(3) - eye(3) / (LambdaN1.')) + lambdaVisco / 2 * (log(JViscoN1) + JViscoN1^2 - JViscoN1) * (eye(3) / (LambdaN1.'));
                DPsiVisDCiN1 = -0.5 * ((LambdaN1.') * DPsiVisDLambdaN1 * GammaN1inv + GammaN1inv * (DPsiVisDLambdaN1.') * LambdaN1);
                if ii == 0
                    DPsiVisDCiN1Resi = DPsiVisDCiN1;
                end
                % stresses
                GammaN1 = -DPsiVisDCiN1; % inelastic stress tensor
                MN1 = 2 * GammaN1 * GammaN1; % Mandel stress tensor

                % 4th order viscous resilience tensor (V^-1):MN05
                VViscoN1 = V1 * MN1.' + V2 * trace(MN1) * eye(dimension);

                Q1 = -2*muVisco*GammaN1 + 2*muVisco/EVisco*SN1
                % local residual and tangent
                if ii == 0 % case ii = 0: compute local residual RL
                    RL = (GammaN1 - GammaN) / DT - 1/eta1 * Q1;
                    RLV = matrixToVoigt(RL, 'strain');
                else % case ii > 0: compute column of local tangent (numerically)
                    RLimag = (GammaN1 - GammaN) / DT - 2 * GammaN1 * VViscoN1;
            ...
            elseif setupObject.newtonVisco.step == 40 || normresi > 10^(10)
                ...
                GammaN1V = GammaN1V - KLL \ RLV;
            end
        end
        obj.epsilonViscoN1(e, k, :) = GammaN1V;
        ...
        for ii = 0:dimR + dimensionVisco
            ...
            elseif ii > dimR
                GammaN1Vakt(ii-dimR) = GammaN1Vakt(ii-dimR) + numTangEps;
            end
            ...
            DPsiComDCxN1 = mu / 2 * (eye(3) - CxN1inv) + lambda / 2 * (log(JN1) + JN1^2 - JN1) * CxN1inv;
            DPsiVisDLambdaN1 = muVisco / 2 * (eye(3) - eye(3) / (LambdaN1.')) + lambdaVisco / 2 * (log(JViscoN1) + JViscoN1^2 - JViscoN1) * (eye(3) / (LambdaN1.'));
            DPsiVisDCxN1 = 0.5 * (DPsiVisDLambdaN1 * GammaN1inv + GammaN1inv * DPsiVisDLambdaN1.');
            DPsiVisDCiN1 = -0.5 * ((LambdaN1.') * DPsiVisDLambdaN1 * GammaN1inv + GammaN1inv * (DPsiVisDLambdaN1.') * LambdaN1);
            ...
            GammaN1 = -DPsiVisDCiN1; % inelastic stress tensor
            MN1 = 2 * GammaN1 * GammaN1; % Mandel stress tensor
            SN1 = 2 * (DPsiComDCxN1 + DPsiVisDCxN1); % 2nd PK stress tensor
            ...
            SN1V = matrixToVoigt(SN1, 'stress'); % 2nd PK stress tensor in Voigt notation
            ...
            if ii == 0 % case ii = 0: compute local residual RL
                RG = RG + BN1.' * SN1V * detJ * gaussWeight(k);
