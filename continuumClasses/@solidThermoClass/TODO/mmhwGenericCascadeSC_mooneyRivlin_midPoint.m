function out = mmhwGenericCascadeSC_mooneyRivlin_midPoint(obj,varargin)
%% Creates the residual and the tangent of the given obj.
%
% Syntax
%
% out = mmhwGenericCascadeSC_mooneyRivlinThermo_midPoint(obj,'PropertyName',PropertyValue)
%
% Description
...
% evaluated at time n+1/2 (midPoint rule).
% 08.06.2017 M.S., A.J., M.F.
...
%         D2eta_thetaN1thetaN1 = -kappa*thetaN1e^(-2);
        Du_CN05 = a*eye(3);
        Du_GN05 = b*eye(3);
        Du_cN05 = c/2*(1 - cN05^(-1/2)) - d/(2*cN05) + DIM*beta*theta0*c/2*cN05^(-1/2);
        Du_thetaN05 = kappa;
        Deta_cN05 =  DIM*beta*c/2*cN05^(-1/2);
        Deta_thetaN05 = kappa/thetaN05e;
        D2u_cN05cN05 = c/4*cN05^(-3/2) + d/2*cN05^(-2) - DIM*beta*theta0*c/4*cN05^(-3/2);
        D2eta_cN05cN05 = -DIM*beta*c/4*cN05^(-3/2);
        D2eta_thetaN05thetaN05 = -kappa*thetaN05e^(-2);
        ...
        rc = kron(N_c(k,:),(Du_cN05 - thetaN05e*Deta_cN05 - SIGMA_cN1));
        ...
        DotC = 1/DT*(CN1 - CN);
        rT = N(k,:)'*((thetaN1e-thetaNe)'/DT + (Deta_thetaN05)^(-1)*Deta_cN05*DotC(map.Voigt)'*(GN05(map.Voigt)'*Isym)') - Du_thetaN05^(-1)*dNx'*QHN05;
        RT = RT + rT*detJ*wp(k);
        ...
        %% Tangent K_c_o
        kcc = (kron(N_c(k,:)'*N_c(k,:),1)*(D2u_cN05cN05 - thetaN05e*D2eta_cN05cN05));
        kcT = -N_c(k,:)'*Deta_cN05*N(k,:);
        ...
        %% Tangent K_T_o
        kTC = N(k,:)'*(Deta_thetaN05)^(-1)*Deta_cN05*kron(1/DT*N_C(k,:),(GN05(map.Voigt))'*Isym);
        mapdNx = [1;2;3;2;3;3];
        kTG = N(k,:)'*(Deta_thetaN05)^(-1)*Deta_cN05*kron(N_G(k,:),(DotC(map.Voigt))'*Isym) + Du_thetaN05^(-1)*k0*cN05^(-1)*kron((Isym*dNx(mapdNx,:))',N_G(k,:));
        kTc = N(k,:)'*(Deta_thetaN05)^(-1)*D2eta_cN05cN05*(DotC(map.Voigt)'*(GN05(map.Voigt)'*Isym)')*N_c(k,:) - k0*cN05^(-2)*(Du_thetaN05)^(-1)*(dNx'*(GN05*(dNx*thetaN05')))*N_c(k,:);
        DQHN05_thetaN05 = -k0*cN05^(-1)*GN05*dNx*eye(size(dNx,2));
        kTT = 2*N(k,:)'*1/DT*N(k,:) - N(k,:)'*Deta_thetaN05^(-2)*(D2eta_thetaN05thetaN05*Deta_cN05)*N(k,:)*(sum(sum(DotC.*GN05))) - Du_thetaN05^(-1)*dNx'*DQHN05_thetaN05;
