function out = dispCascadeSC_mooneyRivlin_endPoint(obj,varargin)
...
for k = 1:NGP
    ...
    DPsi_C = a*eye(3);
    DPsi_G = b*eye(3);
    DPsi_cxN1 = -d/(2*cxN1) + c/2*(1-1/sqrt(cxN1)) - DIM*beta*(thetaN1e - theta0)*(c/2*1/sqrt(cxN1) + d/2*(cxN1)^(-3/2));
