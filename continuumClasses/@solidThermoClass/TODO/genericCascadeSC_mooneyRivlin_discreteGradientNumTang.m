function out = genericCascadeSC_mooneyRivlin_discreteGradientNumTang(obj,varargin)
...
parfor j = 1:numElements
    ...
    [RX,RT,KXX,KTT,KXT,KTX,ePot,flagDiscreteGradient] = Residuum(DT,numDOFs,edN,edN1,thetaN1,thetaN,QREF,edofLocal,dNr,objNT,map,flagDiscreteGradient,0);
...
function [RX,RT,KXX,KTT,KXT,KTX,ePot,flagDiscreteGradient] = Residuum(DT,numDOFs,edN,edN1,thetaN1,thetaN,QREF,edofLocal,dNr,objNT,map,flagDiscreteGradient,flagNumericalTangent)
map.Voigt = [1 5 9 4 8 7]';
%% Shape functions
N = objNT.SHAPEF.N;
wp = objNT.SHAPEF.wp;
...
for k = 1:NGP
    ...
    De_theta = kappa;
    Dentropy_cxN05 =  DIM*beta*(c2/2*1/sqrt(cxN05));
    D2entropy_cxcxN05 = -DIM*beta*(c2/(4*cxN05^(3/2)));
    
    % partitioned greenspan formulas
    if ~flagNumericalTangent
        
        if abs(thetaN1e-thetaNe) >= 1000*eps
        ...
        if abs(cxN1 - cxN) >= 1000*eps
            ...
            DecN1N_cx = - d1/(2*cxN1) + (c1*(cxN1^(1/2) - 1))/(2*cxN1^(1/2)) + DIM*beta*theta0*(c2/(2*cxN1^(1/2)));
            DecN1N1_cx = - d1/(2*cxN1) + (c1*(cxN1^(1/2) - 1))/(2*cxN1^(1/2)) + DIM*beta*theta0*(c2/(2*cxN1^(1/2)));
            ...
            % Energy
            Dec_cx = -d1/(2*cxN05) + c1/2*(1-1/sqrt(cxN05)) + DIM*beta*theta0*(c2/2*1/sqrt(cxN05));
            D2ec_DcxDcx = c1/(4*cxN05^(3/2)) + d1/(2*cxN05^2) - DIM*beta*theta0*(c2/(4*cxN05^(3/2)));
            % 2.P.K
        ...
        if  flagDiscreteGradient(k,2) == true
            % Entropy
            entropyN1N1 = kappa*log(thetaN1e/theta0)+DIM*beta*(c1*(sqrt(cxN1)-1) - d1/sqrt(cxN1));
            entropyN1N = kappa*log(thetaNe/theta0)+DIM*beta*(c1*(sqrt(cxN1)-1) - d1/sqrt(cxN1));
            entropyNN1 = kappa*log(thetaN1e/theta0)+DIM*beta*(c1*(sqrt(cxN)-1) - d1/sqrt(cxN));
            entropyNN = kappa*log(thetaNe/theta0)+DIM*beta*(c1*(sqrt(cxN)-1) - d1/sqrt(cxN));
            Dentropy_thetaLeft = (entropyN1N1 -entropyN1N )*(thetaN1e-thetaNe)^(-1);
            Dentropy_thetaRight = (entropyNN1 -entropyNN )*(thetaN1e-thetaNe)^(-1);
            Dentropy_theta = 1/2*(Dentropy_thetaLeft+Dentropy_thetaRight);
            % Temperature
