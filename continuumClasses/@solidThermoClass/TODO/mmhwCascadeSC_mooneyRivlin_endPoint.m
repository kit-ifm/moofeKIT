function out = mmhwCascadeSC_mooneyRivlin_endPoint(obj,varargin)
...
parfor j = 1:numElements
    ...
    for k = 1:NGP
        ...
        wedge_SIGMA_GN1CN1 = wedge(SIGMA_GN1,CN1);
        wedge_SIGMA_GN1CN1_rC = [wedge_SIGMA_GN1CN1(1,1) wedge_SIGMA_GN1CN1(2,2) wedge_SIGMA_GN1CN1(3,3) 2*wedge_SIGMA_GN1CN1(1,2) 2*wedge_SIGMA_GN1CN1(2,3) 2*wedge_SIGMA_GN1CN1(1,3)].';
        ...
        if TransRSig==1
            rI = kron(N_c(k,:),(DPsi_cN1));
            rI = rI(:);
            rC = kron(N_C(k,:),(DPsi_C_rC+wedge_SIGMA_GN1CN1_rC+1/3*SIGMA_cN1*GN1_rC));
            rC = rC(:);
            rG = kron(N_G(k,:),(DPsi_G_rG+1/3*SIGMA_cN1*CN1_rG));
            rG = rG(:);
        else
            rI = kron(N_c(k,:),(DPsi_cN1-SIGMA_cN1));
            rI = rI(:);
            rC = kron(N_C(k,:),(DPsi_C_rC-SIGMA_CN1_rC+wedge_SIGMA_GN1CN1_rC+1/3*SIGMA_cN1*GN1_rC));
            rC = rC(:);
            rG = kron(N_G(k,:),(DPsi_G_rG-SIGMA_GN1_rG+1/3*SIGMA_cN1*CN1_rG));
            rG = rG(:);
        end
        %% Compatibility SIG
        %rSIG
        ...
        kCSIGc = 1/3*kron(N_C(k,:)'*N_c(k,:),GN1_rC);
        kCG = 1/3*SIGMA_cN1*kron(N_C(k,:)'*N_G(k,:),Isym);
        %% Tangent KG_o
        kGSIGG = -kron(N_G(k,:)'*N_G(k,:),Isym);
        kGSIGc = 1/3*kron(N_G(k,:)'*N_c(k,:),CN1_rG);
        ...
        Q_The = -k0*(1/cN1*GN1*dNx*eye(size(dNx,2)));
        kTT =  N(k,:)'*(etaN1-etaN)*N(k,:)/DT+thetaN1e*N(k,:)'*eta_thetaN1*N(k,:)/DT-dNx'*Q_The;
        %% Tangent K_TG
        I = eye(3);
        Q_1G=zeros(3,3);
        Q_2G=zeros(3,3);
        Q_3G=zeros(3,3);
        Q_1G(:,:)=-0.5*k0*1/cN1*(I(:,1)*(dNx*thetaN1')'+(dNx*thetaN1')*I(1,:));
        Q_2G(:,:)=-0.5*k0*1/cN1*(I(:,2)*(dNx*thetaN1')'+(dNx*thetaN1')*I(2,:));
        Q_3G(:,:)=-0.5*k0*1/cN1*(I(:,3)*(dNx*thetaN1')'+(dNx*thetaN1')*I(3,:));
        Q_Gv = zeros(DIM,6);
        Q_Gv(1,:)=Q_1G(map.Voigt);
        Q_Gv(2,:)=Q_2G(map.Voigt);
        Q_Gv(3,:)=Q_3G(map.Voigt);
        kTG = kron(N_G(k,:),-dNx'*Q_Gv*Isym);
        %% Tangent K_Tc
        Q_c = k0*(1/cN1^2*GN1*(dNx*thetaN1'));
        eta_cN1 = DIM*beta*(c/2*1/sqrt(cN1)+d/2*cN1^(-3/2));
        kTc = N(k,:)'*thetaN1e*eta_cN1*N_c(k,:)/DT-dNx'*Q_c*N_c(k,:);
