function [rData, kData, elementEnergy, array] = dispL2NeoHookeGENERICMidpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    %% residual, energy and tangent
        ...
        % second derivatives
            DDDpsiVolDJJJN05    = bulk * 1 / (JN05^3) * (log(JN05) - 3 / 2);
            DDpsiDevDCxCxN05V   = mu / 2 * ((1 + 2 / 3 * (JN05^2 - JN05)) * sym_voigt(CxN05inv, CxN05inv, 'stress', 'stress') - 1 / 3 * (2*JN05^2 - JN05) * (CxN05invV * CxN05invV.'));
            DDpsi2DJJN05        = 3 * beta * DDDpsiVolDJJJN05;
        ...
        % internal energy and entropy
            ...
            DDentropyDFxFxN05V          = 3 * beta * ((DDDpsiVolDJJJN05 * JN05^2 + DDpsiVolDJJN05 * JN05) * (FxN05invtranspV * FxN05invtranspV.') + DDpsiVolDJJN05 * (-JN05 * (FxN05invtranspV * FxN05invtranspV.') + DiffSecUnsym(FxN05)));
...
%             KTT2_GP     = 1 / DT * N(k,:).' * N(k,:) + 1 / EN05 * k0 * JN05 * (dN_X_I.' * CxN05inv * dN_X_I) +  k0 * JN05 / (EN05^2) * N(k,:).' * (gradEN05 * CxN05inv) * dN_X_I;
