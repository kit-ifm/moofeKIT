function [rData, kData, elementEnergy, array] = displacementHooke3DEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, ~)
% DISPLACEMENTHOOKE3DENDPOINT Element routine of class solidThermoClass.
%
% FORMULATION
...
for k = 1:numberOfGausspoints
    ...
    PsiThermoN = kappa * (thetaNGP - thetaRGP - thetaNGP*log(thetaNGP/thetaRGP));
    ...
    PsiThermoN1 = kappa*(thetaN1GP  - thetaRGP  - thetaN1GP *log(thetaN1GP /thetaRGP ));
    ...
    if ~computePostData
        % residual
        RX = RX + B' * sigmaN1 * detJ * gaussWeight(k);
        RT = RT + (N_k_I(k, :)'*thetaN1GP *(etaN1 - etaN)/DT + dN_X_I' * k0 * dN_X_I * thetaN1(:))*detJ*gaussWeight(k);
        % tangent      
        ...
        KTT = KTT + (N_k_I(k,:)' * ((etaN1 - etaN) / DT * N_k_I(k,:) + thetaN1GP  / DT * kappa /thetaN1GP  * N_k_I(k,:)) + dN_X_I' * k0 * dN_X_I) * detJ * gaussWeight(k);
