function [rData, kData, elementEnergy, array] = mixedPHViscoPTCHyperelasticDiscreteGradient(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
         ...
         % discrete gradient evaluation 
         if (abs(CelN1 - CelN) <= 1e-4) 
             % use midpoint rule if CN1 \approx CN
             DHDCel = DHDCel + Phi(k,:)'*getStrainEnergyDerivative(EAel,CelN05) * detJ * gaussWeight(k);
             D2HDCel2 = D2HDCel2 + Phi(k,:)'*getStrainEnergyHessian(EAel,CelN05)* 1/2 * detJ * gaussWeight(k);

         else
             WtildeN = getStrainEnergy(EAel, CelN);
             WtildeN1 = getStrainEnergy(EAel, CelN1);     
             DHDCel = DHDCel + Phi(k,:)'*(WtildeN1 - WtildeN)/(CelN1 -CelN) * detJ * gaussWeight(k);
             D2HDCel2 = D2HDCel2 + Phi(k,:)'*(getStrainEnergyDerivative(EAel,CelN1)/(CelN1-CelN)-(WtildeN1 - WtildeN)/(CelN1 -CelN)^2) * detJ * gaussWeight(k);
