function [rData, kData, elementEnergy, array] = mixedPHViscoPTCHyperelasticEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
         ...
         K = K + B'*B*rN1*Phi(k, :) * detJ * gaussWeight(k);
         ...
         % use midpoint rule 
         DHDC = DHDC + Phi(k,:)'*getStrainEnergyDerivative(EA,CN1) * detJ * gaussWeight(k);
         DHDCel = DHDCel + Phi(k,:)'*getStrainEnergyDerivative(EAel,CelN1) * detJ * gaussWeight(k);
         D2HDC2 = D2HDC2 + Phi(k,:)'*getStrainEnergyHessian(EA,CN1) * detJ * gaussWeight(k);
         D2HDCel2 = D2HDCel2 + Phi(k,:)'*getStrainEnergyHessian(EAel,CelN1) * detJ * gaussWeight(k);

         % stored strain energy
         elementEnergy.strainEnergy = elementEnergy.strainEnergy + getStrainEnergy(EA,CN1) * detJ * gaussWeight(k) + getStrainEnergy(EA,CelN1) * detJ * gaussWeight(k);
...
elementEnergy.dissipatedEnergy = elementEnergy.dissipatedEnergy + h*(MsInv*DHDCel)'*R_eta*MsInv*DHDCel;
...
if ~computePostData
    ...
    % tangent
    kData{1,1} = 2 * DKDr * MsInv * DHDC + 2 * DKDr * MsInv * DHDCel;
    kData{1,2} = 2 * K * MsInv * D2HDC2;
    kData{1,3} = 2 * K * MsInv * D2HDCel2;
    kData{2,1} = -2 * K' - 2*(DKDr*(rN1-rN))';
    kData{2,2} = Ms;
    kData{2,3} = 0;
    kData{3,1} = -2 * K' - 2*(DKDr*(rN1-rN))';
    kData{3,2} = 0;
    kData{3,3} = Ms + h*2/etaA*D2HDCel2;
