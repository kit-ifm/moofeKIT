function [rData, kData, elementEnergy, array] = mixedPHCHyperelasticViscoEndpoint(obj, setupObject, computePostData, e, rData, kData, dofs, array, stressTensor, flagNumericalTangent)
...
for k = 1:numberOfGausspoints
    ...
    if ~computePostData
         ...
         % visco
         A = A + etaA/CN1 * (B'*B)*(rN1*rN1')*(B'*B) * detJ * gaussWeight(k);
         A2 = A2 + etaA/CN1 * (B'*B)*(rN1*(rN1-rN)')*(B'*B) * detJ * gaussWeight(k);
         DADr = DADr + etaA/CN1 * (B'*B)*((B'*B*rN1)'*(rN1-rN)) * detJ * gaussWeight(k);
         DADC = DADC + -etaA/CN1^2*Phi(k,:) * (B'*B)*(rN1*rN1')*(B'*B) * detJ * gaussWeight(k);
...
if ~computePostData
    ...
    % tangent
    kData{1,1} = 2 * DKDr * MsInv * DHDC + A/h +  DADr / h + A2/h;
